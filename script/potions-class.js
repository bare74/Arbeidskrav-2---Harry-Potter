...
function createNewStudent(arr, i) {
  ...
  if (userAnswer === null || userAnswer.toLowerCase() !== "ja") {
    ...
    studentsActive[i].innerHTML = `
            <img class='student-img' alt='${
              !arr[randomNumber].image
                ? "Missing image avatar"
                : arr[randomNumber].name + " image"
            }' image' src=${
      !arr[randomNumber].image ? "./assets/avatar.png" : arr[randomNumber].image
    } />
            <p>Navn: ${arr[randomNumber].name}</p>
            <p>Hus: ${
              arr[randomNumber].house ? arr[randomNumber].house : "Intet hus"
            }</p>
            ...
            <li class='student-container'>
                <img class='student-img' alt='${
                  !students[student].image
                    ? "Missing image avatar"
                    : students[student].name + " image"
                }' src=${
      !students[student].image ? "./assets/avatar.png" : students[student].image
    } />
                <p>Navn: ${students[student].name}</p>
                <p>Hus: ${
                  students[student].house
                    ? students[student].house
                    : "Intet hus"
                }</p>
                <button class='delete-student-btn'>Slett elev</button>
...
function displayTeacherDetails(data) {
  ...
  severusAge.textContent = `Alder: ${2022 - data[0].yearOfBirth} år`;
}
